        ...
        </div>
        {
          isEdit ?
            ...
            <div className='flex flex-col gap-1'>
              <span className='text-3xl font-bold'>نماد : <input className='inp-e' type="text" defaultValue={data.symbol} /></span>
              <span className='text-zinc-500'>تاریخ : <input className='inp-e' type="text" defaultValue={`${data.time} - ${data.date}`} /> </span>
            </div>
            <div className='flex flex-col gap-1'>
              <span>استراتژی : <input className='inp-e' type="text" defaultValue={data.strategy} /></span>
              {/* <p>توضیحات : <input className='inp-e w-full' type="text" defaultValue={data.notes}/></p> */}
              <span>توضیحات : </span><textarea name="" className='h-fit inp-e' defaultValue={data.note} id=""></textarea>
            </div>
            <div className='flex gap-5'>
              <label htmlFor='rate' className='flex items-center gap-2'>
                <input className='inp-rad' type="radio" name='rate'/>
                <span>TP</span>
              </label>
              <label htmlFor='rate' className='flex items-center gap-2'>
                <input className='inp-rad' type="radio" name='rate'/>
          ...
          <div className='flex flex-col gap-1'>
            ...
            <span className='text-zinc-500'>تاریخ : {data.time} - {data.date}</span>
          </div>
          <div className='flex flex-col gap-1'>
      ...
      </div>
      {
        ...
        <div className='p-6 mx-3 flex flex-col gap-6'>
          <div className='flex w-full flex-wrap gap-4'>
            <input className='inp-e-d border-yellow-600 outline-yellow-600 bg-yellow-200' type="text" defaultValue={`نقطه ورود : ${data.entry}`}/>
            <input className='inp-e-d border-red-600 outline-red-600 bg-red-200' type="text" defaultValue={`حد ضرر : ${data.stopLoss}`}/>
            <input className='inp-e-d border-green-600 outline-green-600 bg-green-200' type="text" defaultValue={`حد سود : ${data.takeProfit}`}/>
            <input className='inp-e-d border-violet-600 outline-violet-600 bg-violet-200' type="text" defaultValue={`حجم معامله : ${data.lotSize}`}/>
            <input className='inp-e-d border-sky-600 outline-sky-600 bg-sky-200' type="text" defaultValue={`ریسک به ریوارد : ${data.riskReward}`}/>
            <input className='inp-e-d border-stone-600 outline-stone-600 bg-stone-200' type="text" defaultValue={`سشن : ${data.session}`}/>
            <input className='inp-e-d border-pink-600 outline-pink-600 bg-pink-200' type="text" defaultValue={`تایم فریم : ${data.timeframe}`}/>
          </div>
          <div className='px-6 py-2 bg-gray-300 text-lg rounded-xl dark:bg-gray-900'>
            ...
            <p className='text-justify' ref={pRef}>ما در این معامله روی ارز {data.symbol} در تاریخ {data.date} با استراتژی {data.strategy} به {explaneResult(data.result)} رسیدیم چون {data.notes}. دراین معامله نقطه ورود ما {data.entry} و نقطه استاپ ما {data.stopLoss} و نقطه سود ما {data.takeProfit} بود که با حجم {data.lot} و ریسک به ریوارد {data.riskReward}  و تایم فریم {data.timeframe} و سشن بازار {data.session} ورود کرده بودیم </p>
          </div>
        </div>
        :
        <div className='p-6 mx-3 flex flex-col gap-6'>
